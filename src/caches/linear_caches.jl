...
@cache struct MMPTMCache{uType,rateType,WType} <: OrdinaryDiffEqMutableCache
  u::uType
  uprev::uType
  uprev2::uType
  tmp::uType
...
function alg_cache(alg::MMPTM,u,rate_prototype,uEltypeNoUnits,
                   tTypeNoUnits,uprev,uprev2,f,t,dt,reltol,p,calck,::Val{true})
  W = false .* vec(rate_prototype) .* vec(rate_prototype)' # uEltype?
  k = zero(rate_prototype); fsalfirst = zero(rate_prototype)
  MMPTMCache(u,uprev,uprev2,similar(u),fsalfirst,W,k)
end

struct MMPTMConstantCache <: OrdinaryDiffEqConstantCache
end
